...
def call_predict_endpoint(module: str, payload: dict) -> dict | None:
    """Posts to /predict/{module} and returns the JSON result, or None on failure."""
    try:
        response = requests.post(f"{API_URL}/predict/{module}", json=payload)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        st.error(f"Request failed for {module}: {e}")
        return None

def _log_trip_request(payload: dict) -> None:
    """Fire-and-forget log of the user's raw trip inputs. Silently no-ops on
    failure so a logging hiccup never blocks the actual trip planning flow."""
    try:
        requests.post(f"{API_URL}/log-trip-request", json=payload, timeout=5)
